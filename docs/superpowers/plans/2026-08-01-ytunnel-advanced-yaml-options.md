...
**Architecture:** Extend `PersistentTunnel` with two `BTreeMap<String, TunnelOptionValue>` fields (`tunnel_options` for top-level YAML keys, `origin_request` for per-ingress-rule `originRequest` keys). Extend the existing `format!`-based `generate_tunnel_config` — no new dependencies, no YAML library. Replace the current chained-modal edit flow (`EditTarget` → `EditZone`) with a two-tab overlay (`Basic` / `Advanced`). No changes to launchd plists or systemd units — cloudflared re-reads the YAML on restart.

**Tech Stack:** Rust 2021, ratatui 0.30, crossterm 0.29, serde 1, toml 0.8. **No new dependencies** — YAML is emitted by hand-written formatter helpers to keep ytunnel's dep tree lean and preserve inspectable, idiomatic output.

**Design Spec:** `docs/superpowers/specs/2026-08-01-ytunnel-advanced-yaml-options-design.md`

**Working directory:** `/Users/ar/ytunnel` (durable location; earlier `/tmp/ytunnel` was cleaned by the OS).

**Testing convention:** Since we're not adding a YAML parser as a dev-dep, tests verify output structure using targeted `contains()` and `matches(...).count()` assertions on the generated string. The generator is deterministic; assertions are stable.

**Project conventions (from CLAUDE.md):** Use `//` comments, not `///` (except when documenting public API surface). Do NOT include `Co-Authored-By: Claude` in commit messages. After code changes, run `cargo install --path .` to update the local binary.
...
**Files modified:**
- `src/state.rs` — add `tunnel_options` + `origin_request` fields; add `TunnelOptionValue` enum; extend `generate_tunnel_config` (format!-based) to render both maps; extract `build_tunnel_yaml` helper for ephemeral reuse.
- `src/tunnel.rs` — replace inline YAML formatting with the shared `build_tunnel_yaml` helper.
...
- `src/tui/mod.rs` — declare `pub mod edit_sheet;`.
- `src/main.rs` — declare `mod cloudflared_options;`.
- `src/daemon.rs` — add `reload_if_installed(tunnel)` that wraps `launchctl kickstart -k` / `systemctl --user restart`.
...
## Task 1: Add TunnelOptionValue and new fields to PersistentTunnel
...
At the top of `src/state.rs`, add to imports:

```rust
use std::collections::BTreeMap;
```

After the imports and before `pub enum TunnelStatus`, add:
...
Then modify the `PersistentTunnel` struct. After the `metrics_port` field, add two new fields:
...
Run: `cargo check`
Expected: clean compile. If any `PersistentTunnel { ... }` construction sites in other modules fail (they use struct-literal syntax without `..Default::default()`), add the two new fields explicitly at each site: `tunnel_options: BTreeMap::new(), origin_request: BTreeMap::new()`.

- [ ] **Step 6: Commit**

```bash
git add src/state.rs && git commit -m "state: add TunnelOptionValue and advanced-options fields to PersistentTunnel"
```

---

## Task 2: Extend generate_tunnel_config to serialize tunnel_options

**Files:**
- Modify: `src/state.rs::generate_tunnel_config` (lines 238-264) and add helper functions.

Extend the existing `format!`-based generator to append top-level YAML keys from `tunnel.tunnel_options`. Introduces the YAML-escape helpers we'll reuse in Task 3.
...
Run: `cargo test --lib state::tests -- --nocapture`
Expected: FAIL — `tunnel_options` map is silently ignored by the current generator; `empty_options_produces_legacy_shape` may pass; the other two fail.

- [ ] **Step 3: Add YAML helper functions**

At the bottom of `src/state.rs` (outside `mod tests`), add:
...
- [ ] **Step 4: Extend the generator to render tunnel_options**

Modify `generate_tunnel_config` (lines 238-264). Replace the current implementation with:
...
```bash
git add src/state.rs && git commit -m "state: render tunnel_options as top-level YAML keys"
```

---

## Task 3: Serialize origin_request as originRequest on the ingress rule
...
Run: `cargo test --lib state::tests::origin_request_attaches_to_hostname_rule_only -- --nocapture`
Expected: FAIL — `origin_request` map is currently ignored.

- [ ] **Step 3: Extend the generator**

In `generate_tunnel_config`, replace this block:
...
Run: `cargo test --lib state::tests -- --nocapture`
Expected: all state tests pass. The `empty_options_produces_legacy_shape` test from Task 2 guards the `if !is_empty` branch from regressing.

- [ ] **Step 5: Commit**

```bash
git add src/state.rs && git commit -m "state: attach origin_request as originRequest on ingress rule"
```

---

## Task 4: Create cloudflared options registry
...
In `src/main.rs`, find the `mod` declarations near the top and add:
...
Run: `cargo test --lib cloudflared_options -- --nocapture`
...
```bash
git add src/cloudflared_options.rs src/main.rs && git commit -m "cloudflared_options: add curated registry of ~30 tunable options"
```

---

## Task 5: Extract build_tunnel_yaml, reuse in ephemeral tunnel

**Files:**
- Modify: `src/state.rs` (extract shared helper)
- Modify: `src/tunnel.rs:20-52` (use shared helper)

Ephemeral tunnels currently build their own inline YAML in `src/tunnel.rs`. Extract the YAML-building logic into a shared function so both paths emit identical shapes.
...
Run: `cargo test --lib state::tests::build_yaml_from_parts_matches_persistent_shape -- --nocapture`
Expected: FAIL — `build_tunnel_yaml` doesn't exist.

- [ ] **Step 3: Extract the shared function**

In `src/state.rs`, add a new `pub fn` above `generate_tunnel_config`:
...
Then replace the body of `generate_tunnel_config` with a thin wrapper:
...
Run: `cargo test --lib state::tests -- --nocapture`
Expected: all pass, including the four prior tests and the new one.

- [ ] **Step 5: Update ephemeral tunnel to use the shared helper**

In `src/tunnel.rs`, replace the section from line 20 (start of `pub async fn run_tunnel`) up through the `fs::write(&config_path, ...)` call with:
...
Leave everything from `// Run cloudflared with the config` onward unchanged.

- [ ] **Step 6: Full build + tests**

Run: `cargo test --lib -- --nocapture && cargo check`
Expected: all pass, clean check.

- [ ] **Step 7: Commit**

```bash
git add src/state.rs src/tunnel.rs && git commit -m "state: extract build_tunnel_yaml; reuse from ephemeral run_tunnel"
```

---

## Task 6: Add daemon reload_if_installed helper
...
```bash
git add src/daemon.rs && git commit -m "daemon: add reload_if_installed for post-edit daemon restart"
```

---

## Task 7: Create the edit sheet module (state + logic)

**Files:**
- Create: `src/tui/edit_sheet.rs`
- Modify: `src/tui/mod.rs` (add `pub mod edit_sheet;`)

This lays the state foundation for the two-tab UI. Rendering + input handling in Task 8.
...
Run: `cargo test --lib tui::edit_sheet -- --nocapture`
Expected: 4 tests pass.

- [ ] **Step 5: Commit**

```bash
git add src/tui/mod.rs src/tui/edit_sheet.rs && git commit -m "tui: add edit_sheet module with state model and tests"
```

---

## Task 8: Wire the edit sheet into the TUI (rendering + input)

**Files:**
- Modify: `src/tui/edit_sheet.rs` (add rendering functions)
- Modify: `src/tui/app.rs` (input modes, key handling, save flow)
- Modify: `src/tui/ui.rs` (dispatch rendering)
...
- [ ] **Step 2: Replace InputMode variants in app.rs**

Open `src/tui/app.rs`. Find `pub enum InputMode`. Remove `EditTarget` and `EditZone`. Add:
...
Then find `pub struct App` and add:

```rust
    pub edit_sheet: Option<crate::tui::edit_sheet::EditSheetState>,
```

Initialize in the constructor as `edit_sheet: None`.

- [ ] **Step 3: Replace start_edit and remove old edit handlers**

Find `fn start_edit`. Replace its body with:
...
If `selected_tunnel_name()` doesn't exist by that exact name, use whatever mechanism the old `start_edit` used to identify the tunnel. Preserve the pattern.

Delete the old `finish_edit_target` / `finish_edit_zone` / `cancel_edit` (or equivalent) functions.

- [ ] **Step 4: Add sheet key handling**

Find the input mode match block (the giant `InputMode::Normal => match key.code { ... }` and its sibling arms). Delete the `InputMode::EditTarget` and `InputMode::EditZone` arms. Add:
...
Add helper functions near the top of `app.rs`:
...
fn commit_input_edit(app: &mut App) {
    ...
    let parsed = match &spec.kind {
        ...
        OptionKind::String { .. } | OptionKind::Duration { .. } => {
            if buffer.trim().is_empty() { None } else { Some(TunnelOptionValue::String(buffer.trim().to_string())) }
        }
        OptionKind::List { .. } => {
            let items: Vec<String> = buffer.lines().map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
...
If `App::set_error` doesn't exist, use whatever error/status-line mechanism the app has. Trace it from existing error paths.

- [ ] **Step 5: Wire rendering in ui.rs**

Open `src/tui/ui.rs`. Find the `InputMode::EditTarget` and `InputMode::EditZone` match arms in the main render dispatch. Replace with:
...
fn render_sheet_modal_overlays(f: &mut Frame, app: &App) {
    match &app.input_mode {
        ...
        InputMode::EditSheetPicker { yaml_key, cursor } => {
            ...
            let inner = block.inner(area);
            f.render_widget(block, area);
            let items: Vec<ratatui::widgets::ListItem> = choices.iter().enumerate().map(|(i, c)| {
...
Delete the now-unused `render_edit_dialog` and `render_edit_zone_dialog` functions.

- [ ] **Step 6: Update the status-bar help lines**

In `src/tui/ui.rs`, find the `InputMode::EditTarget` / `InputMode::EditZone` help strings (around lines 590-591). Delete them and add:
...
- [ ] **Step 7: Build and fix stragglers**

Run: `cargo check`
Expected: may surface leftover references to `EditTarget` / `EditZone` (help screens, other match blocks). Fix each. Iterate until clean.
...
Run: `cargo install --path .` (per CLAUDE.md convention) and then run `ytunnel` in a terminal.

Verify:
- Select a tunnel, press `e` — two-tab sheet opens on Basic.
...
- Ctrl+S — sheet closes. Inspect `~/.config/ytunnel/tunnel-configs/<tunnel>.yml`; confirm it contains `originRequest:` with `httpHostHeader: "google-spike.localhost"` on the ingress rule.
- Re-open edit — value is preserved.
- Select the row, press `d` — value cleared; on next save YAML no longer has the key.
- Press `Esc` after making changes — get discard-confirm dialog. `n` returns, `y` cancels.

- [ ] **Step 10: Commit**

```bash
git add src/tui/ src/state.rs && git commit -m "tui: replace chained edit modals with two-tab sheet (Basic/Advanced)"
```
...
Open `CHANGELOG.md`. Under a new `## [Unreleased]` section (or the topmost version block), add:
...
- Options take effect immediately: saving the edit sheet reloads the running daemon (`launchctl kickstart -k` on macOS, `systemctl --user restart` on Linux).

### Changed
- Tunnel edit UI: replaces the previous target → zone modal chain with a single overlay sheet you can tab between.
- YAML generator extended to render new option keys while preserving byte-for-byte legacy output for tunnels with no advanced options set.
```

- [ ] **Step 2: Commit**

```bash
git add CHANGELOG.md && git commit -m "changelog: advanced tunnel options + edit sheet refactor"
```
...
**Spec coverage:**
- Data model → Task 1
- YAML generation → Tasks 2, 3
- Curated registry → Task 4
- Ephemeral parity → Task 5
- Daemon reload → Task 6
- TUI (edit sheet) → Tasks 7, 8
- Backward compat → Task 1 test (`persistent_tunnel_deserializes_without_new_fields`)
- Legacy YAML shape unchanged → Task 2 test (`empty_options_produces_legacy_shape`)
- No new dependencies → verified by absence of Cargo.toml changes

**Placeholder scan:** no TBDs, TODOs, or "add appropriate X" phrases. Every step has real code.

**Type consistency:** `TunnelOptionValue`, `OptionScope`, `OptionKind`, `OptionSpec`, `EditSheetState`, `SheetTab`, `AdvancedRow` defined once and referenced consistently across state.rs, cloudflared_options.rs, edit_sheet.rs, app.rs, ui.rs.
...
- Snapshot testing for TUI (needs `insta` infrastructure — a dev-dep discussion for a later PR).
